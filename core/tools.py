...
'''
Called in the run loop of Chat and CLiChat (Child class).
In charge of executing the tools requested by Claude in the chat loop. It gets the tool requests from the message returned by Claude, identifies which client can execute each tool, calls the client to execute the tool and then formats the results to be included in the next message sent to Claude.
'''
